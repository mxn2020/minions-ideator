---
name: minions-ideator
id: OC-0100
version: 1.0.0
description: "Agent skills for working with Minions Ideator MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-ideator data."
category: ai
subcategory: general
...
# Minions Ideator Agent Skills

Skills for agents operating on the `minions-ideator` toolbox.
...
```bash
# TypeScript
pnpm add @minions-ideator/sdk

# Python
pip install minions-ideator

# CLI
pnpm add -g @minions-ideator/cli
```
...
The `ideator` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-ideator/sdk/schemas';
...
```python
from minions_ideator.schemas import custom_types
...
- This agent only writes to `minions-ideator` — it reads from other toolboxes but never writes to them